...
- [learn-one-concept](learn-one-concept/SKILL.md)：以轻量互动带用户学懂一个概念，通过讲回、判断或微行动形成证据；用户选择保存时，可在本轮中复用 `concept-card`。
...
- [learn-modeling](learn-modeling/SKILL.md)：把指定材料按章节建成学习者能读懂的学习模型，输出 `learning-models/` 中的材料模型。
- [concept-card](concept-card/SKILL.md)：将笔记中指定的一个概念做成可回溯、可判断和可行动的 Markdown 概念卡。
